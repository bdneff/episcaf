...
C6 mutational ablation series & top-20/mAb $\times$ alanine flavors & $\sim$4000 & spec; to build \\
\bottomrule
\end{tabular}
...
\subsection{C6 --- mutational ablation series}\label{sec:dp4c6}
For the best \num{20} designs of each of the \num{57} mAbs we ship a controlled mutation series
that tests \emph{what} drives any binding observed. Each design appears in up to four flavors:
(i) \textbf{unmutated}, the reference; (ii) the \textbf{first epitope island mutated entirely to
alanine}; (iii) the \textbf{second island to alanine}, where the epitope has one; and (iv)
\textbf{four 6-residue windows of the scaffold} (not the epitope) mutated. The logic is causal.
Ablating an epitope island removes its side-chain chemistry while leaving the fold, so (ii) and
(iii) read out each island's contribution \emph{by mutation} --- the complement to C2, which reads
it out \emph{by presentation} (each island scaffolded alone). Flavor (iv) is the negative control:
if binding is epitope-driven, perturbing the scaffold should leave it intact, so a design whose
binding survives (iv) but collapses under (ii)/(iii) is behaving exactly as intended --- the
epitope, not the scaffold, carries the signal. At \num{20} designs $\times$ \num{57} mAbs $\times$
up to four flavors this is on the order of \num{4000} constructs (flavor (iii) applies only to
multi-island epitopes). \emph{To confirm:} that the mutant residue is alanine throughout, the rule
for choosing the four scaffold windows in (iv), and that ``best 20'' means the top 20 per mAb by
the composite.
