...
separate residue-index map is needed. When a scaffold is too small or too epitope-dense to fit four
disjoint windows, we \emph{fall back to three} (\texttt{scaffoldMutX3}) rather than drop the control,
so a design loses its scaffold control only if it cannot place even three hexamers each $\ge\!4$
residues from the epitope. In the current build the four-window arm covers \num{1043} designs and the
three-window fallback a further \num{43}, so \num{1086} of \num{1120} bases carry a scaffold control
and only \num{34} (\SI{3.0}{\percent}) fit neither; the island-alanine arms cover all bases.
