...
C3 scaffolds, polyclonal tiling & top-\num{3}/window, no antibody & \num{1317} & selected, ungated \\
...
top-\num{20} per group for C1/C2 and top-\num{3} per window for C3 (neighbouring tiles overlap
heavily, so best-\num{3} covers each window). C5 is \num{3000} designs; C4 is the \num{2034} tiles of
Section~\ref{sec:tiling}. Together with the 8VDL arm this is \num{12251} constructs.}
...
scaffold each one (Section~\ref{sec:tiling}), then keep the top \num{3} designs per window by the
composite. We keep only \num{3} (rather than the deeper cut C1/C2 get) because the windows step by
\num{6} residues and so overlap heavily --- adjacent tiles already cover nearly the same epitope
space, so best-\num{3} per window suffices without ballooning the count. This is the polyclonal arm
(Q3): we do not know where the real epitope is, so we cover
...
(4WAT) $+$ \num{276} (6M0J) $=$ \num{1317} top-\num{3} designs (Table~\ref{tab:tilecount}).
...
for C3) assembly emits \num{12251} constructs, each with a unique \texttt{library\_member} and
\texttt{design\_ID}, all \num{103} residues. Selection and depth are settled as above; the remaining
mechanics --- recovering each design's sequence --- are as follows.
