...
(4WAT) $+$ \num{920} (6M0J) $=$ \num{4390} top-\num{10} designs (Table~\ref{tab:tilecount}).
...
rather than by our design, so they are pinned explicitly to the sequences the DP3 order used
(\num{20} nt each, giving a \num{349}-nt oligo) rather than left to a tool default.
...
every row before it goes out --- correct adapters, expected length, and that the coding sequence
translates back to exactly the peptide it claims to encode. The encoder itself is external tooling run
on the cluster, so the computational pipeline ends at the annotated peptide file and this checked order
file is the deliverable that hands off to synthesis.
