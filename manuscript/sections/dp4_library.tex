...
the 2026-07-21 cull and the 2026-07-23 top-up (Section~\ref{sec:assembly}). Together with the 8VDL arm
this is \num{14241} shipped constructs. The delivered \texttt{dp4\_library.csv} additionally carries the
\num{21759} filter-passing LX PfEMP1/EPCR minibinders --- a separate de-novo binder arm, not scaffolded or
scored here --- for a single-file view of the whole library (\num{36000} rows); the \num{14241} above is
the episcaf portion the scoring concerns, while the oligo encoding and order file cover the whole
\num{36000}.}
...
We scaffold it two ways and keep the best \num{29} of each: \texttt{epitope} fixes the whole
...
are antibody-\emph{accessible}: the shipped \num{29} recover the epitope well (epitope RMSD
\SIrange{0.97}{2.16}{\angstrom}) while leaving the Fab surface largely unobstructed (\numrange{0}{3}
clashing residues). The minimal hotspot grafts recover the three residues almost exactly (epitope RMSD
\SIrange{0.03}{0.15}{\angstrom}) but bury them under scaffold that would block the antibody
(\numrange{39}{72} clashing residues). Crucially this is \emph{not} a selection artifact: across all
...
geometry seems to demand occupying the surface the antibody needs. We ship its \num{29} anyway, as
that negative result. This arm is a self-contained component (\texttt{dp4\_8vdl/}); its \num{58} top
designs are merged into the library alongside the others at assembly. (Both arms were originally taken at
a top-\num{10}; the depth rose to \num{29} in the top-up that brought the library onto its \num{36000}
target, Section~\ref{sec:assembly}.)
...
top-\num{10} for C3, top-\num{29} per run for 8VDL) assembly emits \num{14241} episcaf constructs, each with a unique
...
$+$ \num{21759} minibinder); the \num{14241} is what the scoring and selection above concern, while the
...
backbone name from the AlphaFold3 output-directory convention) was collapsed.

\paragraph{The top-up to \num{36000}.} The cull overshot slightly, leaving the library \num{38} peptides
under the \num{36000} target the synthesis budget was set to. Those spare slots went to the 8VDL arm,
whose two runs were deepened from a top-\num{10} to a top-\num{29} each: \num{19} more contiguous-epitope
designs and \num{19} more hotspot designs, taking that arm from \num{20} to \num{58} and the library to
exactly \num{36000}. The depth \num{29} is arithmetic rather than a threshold --- it is how far down the
existing ranking \num{38} spare slots reach when split evenly --- and nothing else changed: the scorer,
the ranking, and every other component's depth are untouched, and the deeper cut is a strict superset of
what had already been selected, so no design that had been chosen was displaced. Going deeper costs
little in quality on the contiguous-epitope arm, where the added designs still sit at zero to three
clashing residues and epitope RMSD below \num{2.2}~\AA; on the hotspot arm it simply adds more of the
same inaccessible designs, which is consistent with that arm's generation-limited behavior
(Section~\ref{sec:dp4_8vdl}) rather than a new failure. Because a top-up renumbers the library members that
follow the inserted block --- the \num{38} additions shift all \num{21759} minibinders --- the order file
was rebuilt by matching each existing oligo to its peptide \emph{by sequence}, so only the \num{38} new
peptides required encoding and every previously computed oligo was carried over unchanged. The final
composition is in Table~\ref{tab:dp4components}.
...
oligo, so the final step converts all \num{36000} 103-mers of the combined library (the \num{14241}
