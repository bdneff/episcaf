...
\subsection{The binding readout: cognate log-enrichment}
...
\subsection{The assayed set: eight antibodies, 377 cognate designs}
...
\subsection{Three limits on what this label can teach}
...
\midrule
cylinder, native-aware & $-0.23$ & $\mathbf{-0.22}$ & $2\times10^{-5}$ & yes --- essentially intact \\
cylinder, plain        & $-0.24$ & $-0.18$ & $3\times10^{-4}$ & mostly \\
...
within-antibody (fixed-effect centered, $n=\num{377}$), with the within-antibody $p$-value. The
accessibility cylinder is the only metric whose correlation barely changes between pooled and
within --- a genuine within-antibody predictor, not a pooling artifact --- and the native-aware
carve is stronger than the plain count. Per antibody, the two well-sampled sets differ: for 7ox3
($n=\num{193}$) both epitope RMSD ($p=0.002$) and the cylinder ($p=0.0009$) are significant, while
for 5fhx ($n=\num{149}$) only the native-aware cylinder reaches significance ($p=0.03$; epitope
RMSD $p=0.16$) --- so the cylinder is the more consistent signal across antibodies.}
...
epitope RMSD carry the ranking weight because they are the two metrics that track binding, while
overall RMSD and PAE weigh less and serve mainly to gate fold quality rather than to rank. What DP3
cannot yet do is \emph{fit} the full composite,
