...
The bare interaction energy recovers the strongest hot spots --- Lys97 is its strongest
antigen call and Lys96 also ranks highly, and Tyr20 and Asp101 also come out favorable --- but it produces a clear false
...
The current twenty-frame estimate does not recover the experimental ranking. Whether this
reflects sparse frame sampling, inadequate conformational sampling, or systematic model error
is not established by this single calculation. It over-cancels the largest hot spot: Lys96's Coulomb ($-34.0$) is
...
\begin{table}[htbp]
\centering
\input{figures/tamarind_pilot_table.tex}
...
$\Delta\Delta G$. Blue and orange identify K96A and R73A in every panel. Every panel includes the same thirteen measured antigen residues, with ML values
from the full scan. The identity line applies only to predicted mutational $\Delta\Delta G$. Simulation
energies are sign-flipped so more favorable contributions point upward, with their own units
and independent vertical scales. Rank correlations use all thirteen measured residues; the earlier two-point pilot is
retained separately and not pooled. Generated by
\texttt{energetics/tamarind/plot\_comparison.py}; plotted values are saved as a CSV.}
\label{fig:tamarind_simulation}
\end{figure}

\paragraph{Full measured antigen scan (same date).}
After the API check, we submitted all thirteen antigen alanine substitutions from the stored
experimental CSV as separate mutants in one new job, retaining the same structure, chain
partners, 20 Monte Carlo samples, and seed 42. Every native identity was verified against the
PDB before submission. The job \texttt{episcaf-v3-3hfm-stabddg-all13-20260908} completed at
\texttt{20:19:41}, after starting at \texttt{20:18:57}: 44 s execution, 156 s queue wait,
and 0.01 reported weighted hours. Frozen artifacts are in
\texttt{energetics/tamarind/3hfm\_all13/}; the original pilot remains separate.
All thirteen original and returned mutation strings agree, the returned original PDB checksum
matches the input, and returned settings match the frozen request.

Across the thirteen mutations, StaB-ddG gives Spearman $\rho=0.746$, Pearson $r=0.845$,
mean absolute error $1.174$ kcal/mol, and root-mean-square error $1.874$ kcal/mol.
The strongest experimental effects are compressed: K96A is predicted at $1.763$ versus $6.49$,
K97A at $1.768$ versus $5.56$, and Y20A at $1.713$ versus $4.27$ kcal/mol.
The ranking correlation is numerically similar to LJ-only ($0.735$), higher than raw interaction
energy ($0.459$), and higher than the current MM-GBSA decomposition ($0.047$). These are
same-system descriptive comparisons, not evidence of statistically established superiority
or independent generalization. No thresholds were fitted to these results.
The full-run K96A and R73A values differ slightly from the pilot despite the same seed; the
comparison uses only full-run predictions, without averaging duplicate runs.

\begin{table}[htbp]
\centering
\input{figures/tamarind_all13_table.tex}
\caption{Full measured antigen alanine scan. Units are kcal/mol. Generated from the separate
full-run archive and experimental CSV by \texttt{report\_pilot.py}, using
\texttt{-{}-run-dir} for \texttt{3hfm\_all13} and
\texttt{-{}-table-name tamarind\_all13\_table.tex}.}
\label{tab:tamarind_all13}
\end{table}

The figure is regenerated with \texttt{plot\_comparison.py -{}-run-dir} pointing to
\texttt{3hfm\_all13}. Its exact commands are in the figure ledger and Tamarind README;
\texttt{plot\_points.csv} retains every plotted value and \texttt{plot\_statistics.json}
retains the reported correlations and errors. The upstream model's SKEMPI fine-tuning means
that training overlap must be resolved before this is called an independent benchmark.
Additional complexes remain necessary before using a learned or simulation-derived signal
