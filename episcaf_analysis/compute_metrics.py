...
def find_af3_files(af3_dir: Path) -> Tuple[Optional[Path], Optional[Path], Optional[Path]]:
    ...
    Important: *_confidences.json glob also matches *_summary_confidences.json, so we
    explicitly exclude summary files when looking for the full confidences file.
    """
    cif = next(af3_dir.glob("*_model.cif"), None) or next(af3_dir.rglob("model.cif"), None)

    # Full confidences file (contains PAE matrix) — must NOT be a summary file
    conf = None
...
def run_metrics(args: argparse.Namespace) -> None:
    ...
    print(f"[metrics] dp2 rows (all)   : {len(dp2):,}")
    print(f"[metrics] RFD3 index       : {len(rfd3_idx):,}")
    print(f"[metrics] AF3 index        : {len(af3_idx):,}")

    # Filter dp2 to only tokens present in this run
    run_tokens = set(tok for tok, _ in rfd3_idx.keys())
    dp2 = dp2[dp2["assay_scaffolded_epitope_id"].isin(run_tokens)].copy()
    print(f"[metrics] dp2 rows (in run): {len(dp2):,}")
