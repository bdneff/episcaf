#!/usr/bin/env bash
# run_whole_epitope_mpnn_af3.sh -- stage 03_mpnn + 04_af3 for the C1 (whole-epitope, 103) run.
#
# Run AFTER the 02_rfd3 array has finished (all backbones in 02_rfd3/outputs). It does the cheap
# login-node fixed-PDB step, then prints the two GPU sbatch waves in order. It does NOT submit --
# you run the printed sbatch lines, and MPNN (wave 1) must finish before AF3 (wave 2), since AF3
# reads the MPNN sequences. Identical to run_dual_island_mpnn_af3.sh but pointed at the
# whole-epitope ledger (the stages are generic on the ledger schema).
#
...
LEDGER_CSV="${LEDGER_CSV:-results/whole_epitope_designs.csv}"
...
Next, the two GPU waves (run on gemini; MPNN must finish before AF3):
  ...
  #   chunk under MaxArraySize if TASKS is large:
