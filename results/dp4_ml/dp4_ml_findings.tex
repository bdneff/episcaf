...
metric-space titration (C5, 169) and epitope-control (C6, 89) arms, whose composite is left blank in the
library file (their metrics are in the superset), so the model does not rank them here. C5 is the honest
unselected baseline: it spans the metric space on purpose and binds at 6.2\% (169/2{,}715), against 15.3\%
for the top-ranked C1 whole-epitope designs, so selection over an unselected background raises the hit
rate about 2.5-fold before any learning. Epitope structure matters for how we score every model: we always
