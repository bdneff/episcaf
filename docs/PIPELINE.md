...
| 8 | Assemble library | local | `scripts/stage06_assemble.py --depth 20` → `data/libraries/dp4_library.csv` (14,241 episcaf) | Concatenate the seven episcaf components; 56-mAb exclusion + depth cuts + global numbering. Ships the full 33-column schema. `--vdl` selects the 8VDL depth (shipped: top-29/run). |
| 8b | Fold in minibinders | local | `dp4_8vdl/scripts/08_add_minibinders.py --lx <LX.csv>` → 36,000 rows | Append the 21,759 passing LX minibinders (with their `lx_*` columns) so the file is the whole DP4 library. Idempotent. |
| 9 | Export encoder input | local | `scripts/stage07_named_peptides.py` → `dp4_named_peptides.csv` | `name,seq`, no header, all 36,000 (add `--exclude-category minibinder` for episcaf-only). **LF endings only** — CRLF makes the encoder silently reject every peptide. When the library only *grew*, use `scripts/stage07_new_peptides.py` to encode just the new ones. |
