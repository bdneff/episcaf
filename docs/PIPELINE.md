...
| 7 | Score + select | local | `scripts/stage06_select.py --preset antibody\|twelvemer --group … --topk …` (wraps `episcaf_analysis.score`) | Rank, no hard gate. C5 titration is `stage06_sample_c5.py`. Dials in `presets.py`. |
...
| 10 | Encode → order file | cluster | `oligo_encoding/encode_step1_generate.sbatch` → `encode_step2_select.sbatch`; then `scripts/stage07_order_file.py` | Peptide → DNA (LadnerLab, DP3 codon weights) → Twist-adapter order file, row-verified. See `oligo_encoding/README.md`. |
